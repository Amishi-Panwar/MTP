...
//		}
		...
		//print main method
		SootClass iclass = Scene.v().loadClassAndSupport("Interning4");
		SootMethod mainMethod = iclass.getMethodByName("main");
//		System.out.println("___Main Method____");
//		System.out.println(mainMethod.retrieveActiveBody());
		
//		System.out.println("___PRINT FIELDS OF DEMOGRAPHICS____");
//		SootClass dclass = Scene.v().loadClassAndSupport("Demographics");
//
//		System.out.println(dclass.getFieldCount());
         
		//run loop for all classes to be interned
...
//	    				for (Unit unit : body.getUnits()) {
	    					if (unit instanceof AssignStmt) {
	    		                ...
	    		                if (rightOp instanceof NewExpr) {
	    		                    ...
	    		                    if(instantiatedClass.equals("Demographics")) {
	    		                    	units.remove(unit);
	    		                    	if(iter.hasNext()) {
	    		                    	...
	    		                    	SootClass sootClass = Scene.v().loadClassAndSupport("Demographics");
	    		                    	
	    		                    	SootMethod sm = sootClass.getMethodByName("create_object2");
	    		                    	InvokeExpr createObjectExpr = Jimple.v().newStaticInvokeExpr(
	    		                                 sm.makeRef(),
	    		                                 constructorArgs
	    		                         );
	    		                    	
//	    		                    	InvokeExpr createObjectExpr1 = Jimple.v().newStaticInvokeExpr(
//		                                Scene.v().getMethod("<Demographics: Demographics create_object2(int, java.lang.String)>").makeRef(),
//		                                constructorArgs
//	    		                    			);
	    		                    	
//	    		                        InvokeExpr createObjectExpr = Jimple.v().newStaticInvokeExpr(
//	    		                                Scene.v().getMethod( instantiatedClass + ": " + instantiatedClass + " create_object" + invokeExpr.getArgCount() +"(int, java.lang.String)>").makeRef(),
//	    		                                constructorArgs
//	    		                        );
//	    		                    	
...
//	    		                        InvokeExpr createObjectExpr = Jimple.v().newStaticInvokeExpr(
//	    		                                Scene.v().getMethod("<Demographics: Demographics create_object2(int, java.lang.String)>").makeRef(),
//	    		                                methodArgs // Assuming create_object2 method takes int and String arguments
//	    		                        );
...
//		}
	...
	public static synchronized void addMultipleArgumentConstructor(SootMethod constructor, String className) {
        ...
        List<Type> parameterTypesList = new ArrayList<>(constructor.getParameterTypes());
...
//-------------------------------------------------------------------------------------------------------       
        ...
        SootMethod createObjectMethod = new SootMethod(createObjectMethodName, parameterTypesList, Scene.v().getSootClass(className).getType() , Modifier.PUBLIC | Modifier.STATIC);
...
//        System.out.println(body);
	...
	public static synchronized void addFactoryMethodUtil(SootMethod constructor, String className, Chain<SootField> allFields) {
        ...
        SootMethod createObjectUtil = new SootMethod(createObjectMethodName, constructor.getParameterTypes(), VoidType.v() , Modifier.PUBLIC | Modifier.STATIC);

        sootClass.addMethod(createObjectUtil);
        
        JimpleBody body = Jimple.v().newBody(createObjectUtil);
...
//	 Local thisLocal = Jimple.v().newLocal("this", Scene.v().getSootClass(className).getType());
//	 body.getLocals().add(thisLocal);
	 
	 Body constructorBody = constructor.getActiveBody();
	 List<Local> constructorParameterList = new ArrayList<>(constructorBody.getParameterLocals());
...
//                        constructorBody.getLocals().add(lhsLocal);
                    ...
                    AssignStmt newAssignStmt = Jimple.v().newAssignStmt(lhsLocal, assignStmt.getRightOp());
                    System.out.println("newAssi "+newAssignStmt);
//                        units.swapWith(unit, newAssignStmt);
